...
int main( int argc, char** argv){
	...
	cout << "Partition Number: " << pNum << endl;
	...
	R_Graph r_g;
	{
		...
		if (!feof(gr_f)) {
			fscanf_s(gr_f, "%u %u\n", &a_g.nodeNum, &a_g.edgeNum);
		}
		cout << "Node number: " << a_g.nodeNum << " ; Edge number: " << a_g.edgeNum << endl;

		a_g.partitionNum = pNum;
		cout << "Partition number: " << pNum << endl;

		sz_lim = a_g.nodeNum / a_g.partitionNum;
		VERBOSE(printf("Node size limit: %d\n", sz_lim); )
