...
void G_Graph::read_graph(string co_path, string gr_path){
		...
		if (nodeNum != node_count) {
			cout << "Inconsistent node number!" << endl;
		}

		cout << "Node number: " << nodeNum << " ; Edge number: " << edge_count << endl;
		...
		while (!feof(gr_f)) {
			...
			this->node_list[ts].get_adj_list().push_back((G_Edge*)&(this->edge_list.back()));
		}
		fclose(gr_f);
...
void G_Graph::convert_n_output( string r_path ){
		...
		FILE *node_f, *edge_f;
		string node_n = to_string(partitionNum)+"anode.txt";
		string edge_n = to_string(partitionNum)+"aedge.txt";
